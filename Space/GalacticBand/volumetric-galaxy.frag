// volumetric-galaxy.frag — the Milky Way as a volumetric disk we are inside.
//
// A world-space volume (so it has parallax): an exponential disk plus a central bulge,
// lit from the inside and self-absorbing. The band is simply the long line of sight
// through the plane; the brightness toward the core is the geometry, not a mask.
//
//     vec3 galacticBandSky(vec3 camPos, vec3 dir, float pxPerDir, out vec3 transmittance);
//
// Emission plus chromatic transmittance. Compose it as background content, beneath the
// local starfield.
//
